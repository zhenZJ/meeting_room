...
<body>
  <article class="page-container">
    <form action="addAction" method="post" class="form form-horizontal" id="form-member-add">
      <div class="row cl">
        ...
        <div class="formControls col-xs-8 col-sm-9 skin-minimal">
          <c:if test="${!empty roleList}">
            <c:forEach var="role" items="${roleList }" varStatus="varStatus">
              <div class="radio-box">
                <input name="roleId" type="radio" id="sex-${role.roleId}" value="${role.roleId }" <c:if test="${role.roleId eq 2 }">checked</c:if>  > <label for="sex-1">${role.roleName }</label>
              </div>
            </c:forEach>
          </c:if>
        </div>
